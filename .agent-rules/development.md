...
每次提交前必须逐项确认以下条件，按顺序执行：

### 第一步：判断是否影响对外行为

如果本次改动涉及以下任意一项，必须同步更新 `CHANGELOG.md`：

- [ ] 新增、修改或删除了组件的 Props
- [ ] 新增、修改或删除了组件的事件（`@event`）
- [ ] 新增、修改或删除了导出函数（`src/main.js` 中的 export）
- [ ] 新增、修改或删除了配置项（`src/config/default.js`）
- [ ] 新增、修改或删除了 AI actions 白名单
- [ ] 新增、修改或删除了编辑器内置扩展

### 第二步：CHANGELOG 同步

如果第一步判断为"是"，则在 `CHANGELOG.md` 顶部新增版本条目，格式如下：

```markdown
## {MAJOR}.{MINOR}.{PATCH} (YYYY-MM-DD)

### 新增功能 / 行为变化 / Bug 修复 / 架构调整 / 文档更新
- 具体改动描述
```

版本号递增规则（参见 `release-docs.md`）：
...
- [ ] `npm test` 通过（涉及组件/逻辑变更时）
- [ ] `npm run build` 通过（涉及构建/打包变更时）
- [ ] `npm run docs:build` 通过（涉及文档变更时）
