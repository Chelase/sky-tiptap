...
- 将示例应用挂载逻辑从库入口中拆出
- 保留一个纯净的公共导出入口
- 检查 `src/index.vue` 是否仍有保留必要
- 检查 `src/utils/index.js` 是否可以删除或转发到新配置
...
- 评估是否保留全局 `window.skyTiptapEditor`
...
如需更强执行性，也可以后续补一份面向实施的 `REFACTOR_TODO.md`，按文件和任务拆分到可直接动手的粒度。
...
优先级最高的工作不是继续叠加新功能，而是先把入口职责、重复实现、SSR 风险和文档一致性整理干净。

在这些问题收束之后，后续再做体积优化、发布治理和功能扩展，成本会明显更低，风险也更可控。
