...
|---------|----------|----------|------|
| 2.1 | 创建新的目录结构 | 低 | ✅ 已完成 |
| 2.2 | 迁移现有组件到新结构 | 中 | 🔄 进行中 (50%) |
| 2.3 | 更新导入路径和引用 | 中 | ⏳ 待执行 |
| 2.4 | 确保向后兼容（导出 API 不变） | 中 | ⏳ 待执行 |

### 已完成子任务
- ✅ 创建样式系统（variables.css, base.css）
...
| 任务 ID | 任务内容 | 风险等级 | 状态 |
|---------|----------|----------|------|
| 3.1 | 建立 CSS 变量系统 | 低 | ⏳ 待执行 |
| 3.2 | 实现主题切换机制（data-theme 属性） | 中 | ⏳ 待执行 |
| 3.3 | 支持自定义样式注入（customStyle prop） | 中 | ⏳ 待执行 |
| 3.4 | 参考截图重绘工具栏样式 | 中 | ⏳ 待执行 |
| 3.5 | 参考截图重绘编辑器区域样式 | 中 | ⏳ 待执行 |
| 3.6 | 参考截图重绘悬浮菜单样式 | 中 | ⏳ 待执行 |
...
|---------|----------|----------|----------|------|
| 4.1 | 移除标题功能 | 删除 Heading 扩展和相关 UI | 低 | ⏳ 待执行 |
| 4.2 | 移除字符限制 | 删除字数统计逻辑 | 低 | ⏳ 待执行 |
| 4.3 | 移除应用/商品卡片 | 从插入菜单删除相关选项 | 低 | ⏳ 待执行 |
| 4.4 | 使用内联 SVG 图标 | 替换现有图标方案 | 中 | ⏳ 待执行 |
| 4.5 | 简化插入菜单 | 只保留：图片、视频、代码块、分割线、表格、链接 | 低 | ⏳ 待执行 |
| 4.6 | 添加悬浮菜单 | 选中文字时显示格式化选项 | 中 | ⏳ 待执行 |
...
| 任务 ID | 任务内容 | 状态 |
|---------|----------|------|
| 5.1 | 开发环境测试（npm run dev） | ⏳ 待执行 |
| 5.2 | 构建测试（npm run build） | ⏳ 待执行 |
| 5.3 | 基础功能验证 | ⏳ 待执行 |
| 5.4 | 样式主题切换测试 | ⏳ 待执行 |
