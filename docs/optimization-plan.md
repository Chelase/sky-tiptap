...
项目目前使用 `mitt` 做内部事件通信（`src/utils/emitter.js`）。

它主要用于：

- 子组件（CustomParagraphComponent、InsertMenu）触发主组件（SkyTiptap）的操作
- 工具栏、插入菜单、自定义段落之间通信
- 子组件通知主组件打开文件选择器、触发 AI 生成等

### 评估结论

**保持 mitt**，理由：

1. **场景合适**：子组件需要调用父组件的方法，这是典型的事件总线场景
2. **代码清晰**：当前实现逻辑清晰，事件命名语义化（`trigger-add-image`、`AI-generated` 等）
3. **收益有限**：替换为 `provide/inject` 或 `ref` 只是代码风格变化，不增加功能
4. **风险大于收益**：替换可能引入 bug，需要大量测试验证
